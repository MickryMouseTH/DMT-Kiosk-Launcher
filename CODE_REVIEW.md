...
> เอกสารตรวจสอบโค้ดและคู่มืออ้างอิง · เวอร์ชันโปรแกรม **2.3**
...
| `Recovery_Enabled` | `1` | `1` = เปิด recovery chord (กดหลายปุ่มพร้อมกันออกโดยไม่ใช้รหัส), `0` = ปิด |
| `Recovery_Combo` | `["1","2","3","4","7","8","9","0","f","h"]` | ชุดปุ่มที่ต้องกด**ค้างพร้อมกันทั้งหมด**เพื่อ force-exit (ดูข้อ 11) |
...
## 10. ตรวจสอบ Logic เทียบ Requirement คีออสค์ (2026-06-12)

**Requirement ที่ตรวจ:**
1. ล็อก app เป็น fullscreen — ไม่ให้โปรแกรมอื่นเปิดซ้อน
2. popup / process ลูกที่ app เรียกเอง ต้องทำงานได้ปกติ
3. คีย์บอร์ดใช้งานได้ทั้งหมด แต่ **ปิดโปรแกรมไม่ได้ / สลับไปโปรแกรมอื่นไม่ได้** จนกว่าจะกด `Ctrl+Alt+Q` + ใส่รหัสผ่าน

### ช่องโหว่ที่พบและแก้แล้ว

| รหัส | ระดับ | ปัญหา | การแก้ |
|------|-------|-------|--------|
| **W1** | 🔴 | เช็คแค่ `fg_pid == _target_pid` → ถ้า app เปิด **process ลูก** (helper/dialog/engine) จะถูกมองเป็นโปรแกรมแปลกปลอม → monitor แย่ง focus กลับมาทับ popup ของ app เองทุกวินาที (ขัด req 2) | ✅ เพิ่ม `refresh_target_family()` cache PID ทั้ง process tree (refresh ทุกรอบ monitor) — ใช้ทั้งใน monitor loop และ keyboard hook |
| **W2** | 🔴 | **Alt+Esc ไม่ถูกบล็อก** — สลับหน้าต่างหนีออกจากคีออสค์ได้ทันที (ขัด req 3) | ✅ บล็อก Alt+Esc เสมอ (รวม Ctrl+Esc / Ctrl+Shift+Esc=Task Manager ในกฎเดียวกัน) |
| **W3** | 🔴 | ระหว่าง dialog รหัสผ่านเปิด hook ปล่อย**ทุกคีย์** → กด `Ctrl+Alt+Q` แล้ว **Alt+Tab หนีได้โดยไม่ต้องรู้รหัส** (ขัด req 3) | ✅ ตอน `_monitor_paused` ยังพิมพ์ได้ทุกตัวอักษร แต่บล็อก Alt+Tab / Alt+Esc / Ctrl+Esc เสมอ |
| **W4** | 🟠 | `SetForegroundWindow` จาก background process มักโดน Windows foreground-lock ปฏิเสธ → ดึง focus กลับล้มเหลวเงียบ ๆ (req 1 ไม่แน่นอน) | ✅ เพิ่ม `force_foreground()` — ลองตรง ๆ ก่อน ถ้าโดนปฏิเสธจำลองกด ALT แล้วลองใหม่ |

### สรุปการป้องกันคีย์บอร์ดหลังแก้ (req 3)

| คีย์ | พฤติกรรม |
|------|----------|
| ตัวอักษร/ตัวเลข/Numpad/F-keys | ✅ ใช้ได้ปกติทั้งหมด (Numpad whitelist เสมอ) |
| Alt+F4, Ctrl+W | 🚫 บล็อกเมื่อหน้าต่างของ app (รวม process ลูก) อยู่หน้า |
| Alt+Tab, Ctrl+Alt+Tab, Alt+Esc, Ctrl+Esc, Ctrl+Shift+Esc | 🚫 บล็อกเสมอ (รวมตอน dialog รหัสผ่านเปิด) |
| Win key (ซ้าย/ขวา/Apps) | 🚫 hard-block ตลอด → Win+combo ทั้งหมดตายไปด้วย |
| `Ctrl+Alt+Q` | ✅ เปิด dialog รหัสผ่าน (ทางออกเดียว) |
| Ctrl+Alt+Del | ⚠️ บล็อกไม่ได้ (SAS ระดับ OS) — ต้องจัดการผ่าน Group Policy ถ้าต้องการ |

### ข้อจำกัดที่เหลือ (ทราบแล้ว ยอมรับได้)
- **Ctrl+Alt+Del** บล็อกใน user-mode ไม่ได้ → แนะนำ GPO: ปิด Task Manager / Lock / Sign out ผ่าน `Software\Microsoft\Windows\CurrentVersion\Policies\System`
- หน้าต่างจากโปรแกรมอื่นที่เด้งขึ้นเอง จะถูกดันกลับภายใน ≤ `Watch_Interval` (1 วินาที) — ไม่ใช่ทันที
- ถ้า target EXE เป็นตัว launcher ที่เปิด process จริงแล้วตัวเองปิด → monitor จะมองว่า app ตายและ restart วน (ใช้ EXE ตัวจริงเป็น `EXE_Path` เสมอ)

---

## 11. Recovery Chord — ทางออกฉุกเฉินกันลืมรหัสผ่าน (2026-06-12)

ฟีเจอร์ "กดหลายปุ่มพร้อมกัน" เพื่อ force-exit คีออสค์ **โดยไม่ต้องใช้รหัสผ่าน** — เผื่อกรณีลืม/ทำรหัสหาย

### วิธีใช้
กด**ค้างพร้อมกันทั้งหมด**ตามที่ตั้งใน `Recovery_Combo` (default: **`1 2 3 4 7 8 9 0 f h`** — 10 ปุ่ม)
→ โปรแกรมปิด target + ออกจากคีออสค์ทันที (log เป็น `CRITICAL`)

### ทำงานอย่างไร
| ส่วน | รายละเอียด |
|------|-----------|
| ตรวจจับ | keyboard hook track ปุ่มที่กดค้างใน `_pressed_keys` (เพิ่มตอน down / ลบตอน up) |
| เงื่อนไข | เมื่อ `RECOVERY_COMBO.issubset(_pressed_keys)` = ปุ่มในชุดถูกกดค้างครบทุกตัว |
| ทำงานทุกโหมด | ตรวจ**ก่อน** logic อื่น → ใช้ได้ทั้งตอนคีออสค์ปกติ และตอน dialog รหัสผ่านเปิดอยู่ |
| one-shot | `_recovery_triggered` กันยิงซ้ำ |
| ปิดงาน | `do_recovery_exit()` รันใน thread แยก (ไม่บล็อก hook) → `_running=False` → restore + `terminate_target()` |
| ปิด dialog | ถ้า dialog เปิดอยู่ `continuous_lift` เห็น `_running=False` แล้วปิดตัวเอง → mainloop จบ |

### ⚠️ ข้อจำกัดสำคัญ — Keyboard Rollover / Ghosting
**คีย์บอร์ดส่วนใหญ่ลงทะเบียนพร้อมกันได้แค่ 6 ปุ่ม (6KRO)** หรือน้อยกว่า (membrane ราคาถูก) — การกด 10 ปุ่มพร้อมกันอาจ**ไม่ทำงาน**บนฮาร์ดแวร์จริง
- ✅ ต้อง**ทดสอบบนคีย์บอร์ดของเครื่องคีออสค์จริง** ก่อน deploy
- ถ้ากดครบแล้วไม่ทำงาน → **ลดจำนวนปุ่ม** ใน `Recovery_Combo` เหลือ 4–6 ปุ่ม (แนะนำเลือกปุ่มที่อยู่คนละโซน เช่น 4 มุมคีย์บอร์ด เพื่อลด ghosting)
- คีย์บอร์ด NKRO (n-key rollover) เท่านั้นที่รองรับ 10 ปุ่มได้แน่นอน

### ⚠️ ข้อพิจารณาด้านความปลอดภัย
Recovery chord เป็น **backdoor ที่ข้ามรหัสผ่าน** — ใครรู้ชุดปุ่มก็ออกจากคีออสค์ได้
- ตั้งชุดปุ่มที่ **กดโดยบังเอิญยาก** และ **เก็บเป็นความลับ** เหมือนรหัสสำรอง
- ทุกครั้งที่ใช้จะถูก log `CRITICAL` ไว้ตรวจสอบย้อนหลัง
- ปิดได้ด้วย `"Recovery_Enabled": 0` ถ้าไม่ต้องการ backdoor (แต่เสี่ยง lockout ถาวรถ้าลืมรหัส)

### Trade-off ที่เลือก
| ตัวเลือก | ผล |
|----------|-----|
| `Recovery_Enabled: 1` (default) | กันลืมรหัสได้ แต่มี backdoor |
| `Recovery_Enabled: 0` | ปลอดภัยสูงสุด แต่ลืมรหัส = ต้อง kill ผ่าน remote/safe mode เท่านั้น |

### ตรวจสอบ Logic เชิงลึกรอบ 2 (2026-06-12)

ไล่ตรวจ recovery chord ทุก path + race condition พบและแก้เพิ่ม:

| รหัส | ระดับ | ปัญหา | การแก้ |
|------|-------|-------|--------|
| **R1** | 🟠 กลาง | recovery chord ไม่ได้ park monitor (`_monitor_paused`) ต่างจาก password path → เกิด race: monitor เข้า loop body ตอน `_running=True` แล้ว recovery thread เซ็ต `_running=False` + terminate (`_target_proc=None`) → monitor อ่าน `_target_proc is None` → **`start_target()` เปิด app ใหม่** เหลือ orphan process | ✅ เพิ่ม `if not _running: break` ก่อน `start_target()` ใน monitor_loop + เซ็ต `_monitor_paused=True` ใน `do_recovery_exit` |
| **R2** | 🟡 ต่ำ | recovery ตอน dialog เปิดอยู่ → `handle_emergency` else-branch พยายาม re-assert TOPMOST บนหน้าต่างที่กำลังถูก terminate | ✅ เปลี่ยน `else:` เป็น `elif _running:` ข้ามการ restore เมื่อกำลัง shutdown |
| **R4** | 🟡 ต่ำ | `int(config['Recovery_Enabled'])` crash ได้ถ้าค่าใน config เป็น string แปลก ๆ + `Recovery_Combo` ไม่ใช่ list | ✅ wrap try/except + ตรวจ `isinstance(list)` |

### ✅ Logic ที่ตรวจแล้วถูกต้อง (จำลอง event sequence ผ่านหมด)
- กดครบ 10 ปุ่มค้าง → fire ตอนปุ่มที่ 10 พอดี (timing-robust ไม่ต้องกดเป๊ะพร้อมกัน)
- auto-repeat (down ซ้ำขณะถือค้าง) → ไม่ยิงซ้ำ (`_recovery_triggered` one-shot)
- พิมพ์ 1..0,f,h ทีละตัวแล้วปล่อย → ไม่ fire (ไม่ครบพร้อมกัน)
- มีปุ่มอื่นกดค้างปนด้วย → ยังตรวจ `issubset` ถูก (extra keys ไม่กวน)
- `_pressed_keys` ติดตามแยกจาก suppression logic (track ก่อน return) → นับถูกแม้คีย์นั้นถูกบล็อก
- recovery ตอน dialog เปิด → `continuous_lift` เห็น `_running=False` ปิด dialog เอง → mainloop จบ → exit สะอาด

### ⚠️ R3 (ต่ำ, รับทราบ) — stuck-key ข้าม secure desktop
ตอน UAC / Ctrl+Alt+Del / สลับ session, LL hook อาจไม่ได้รับ event `up` ของปุ่มที่ถือค้างอยู่ → ปุ่มนั้น "ค้าง" ใน `_pressed_keys` ทำให้กำแพงกัน trigger บังเอิญต่ำลงเล็กน้อย
- ความเสี่ยงต่ำมาก (ต้องบังเอิญถือปุ่มใน combo ค้างพอดีตอน transition)
- ถ้ากังวล: ลดจำนวนปุ่ม combo หรือเลือกปุ่มที่ไม่ค่อยถือค้าง
